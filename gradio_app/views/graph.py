"""Vue graphe des thèmes, alimentée par la base.

Sert une page maison (views/static/) qui charge Plotly et écoute plotly_click :
gr.Plot n'expose pas d'évènement de clic.

Les données sont lues une fois au démarrage : 6788 topics et 9579 détections
tiennent en mémoire, ce qui évite une requête récursive à chaque interaction.
"""
...
from data_helpers import charger_detections, charger_taxonomie
...
# --- 1. données ---
_tax = charger_taxonomie()
_det = charger_detections()

by_name = {r.name: r for r in _tax.itertuples()}
parent_nom = {r.name: (r.parent_nom if r.parent_nom else None) for r in _tax.itertuples()}

occ = defaultdict(list)
for r in _det.itertuples():
    occ[r.topic].append((r.external_doc_id, r.summary, r.verbatim))
...
def _budgets(racines, poids):
    ...
    calibré sur des milliers de feuilles alors qu'on en dessine quelques dizaines."""
